...
/* found an avaliable node */			
    TEST_PRINT(
        "mi_pool_alloc: found an avaliable node at 0x%08X, list at 0x%08X", 
        new_node, 
        list);
    if (!prev) {
        /* insert to head */		
        TEST_PRINT(
            "mi_pool_alloc: insert to head 0x%08X", 
            list);
        parent->data = list->next;
    }
    else {
        TEST_PRINT(
            "mi_pool_alloc: adjust list prev 0x%08X, list 0x%08X", 
            prev, 
            list);
        prev->next = list->next;	
    }
...
static void mi_pool_free (mi_pool_t *pool)
{
    ...
    if (!pool->parent){
        /* root pool */	
        TEST_PRINT(
            "mi_pool_free: root pool at 0x%08X", 
            pool);
        ...
        if (pool->data && pool->data->data){
            TEST_PRINT(
                "mi_pool_free: child pool at 0x%08X of parent: 0x%08X", 
                pool, 
                pool->parent);
...
/* we need alloc a new mem node list */
    new_node_list = 
        _create_mem_node_list(pool, 
                              size + sizeof(_mi_mem_node_head_t));
...
/* TODO */
    head = 
        (_mi_mem_node_head_t*)((mbyte*)data-sizeof(_mi_mem_node_head_t));
